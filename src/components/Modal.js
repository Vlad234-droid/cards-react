...
const Modal = ({
	...
	const addToCartHandler = (indexOfCard, data) => {
		const dataFilter = data.filter((_, i) => i === indexOfCard);
		const infoOfCard = dataFilter;

		setCart((cart) => [...cart, ...infoOfCard]);
		setDisplay(!display);
	};
		...
		<CardShadow className="shadow" onClick={exitCardHandler}>
			<Card style={{backgroundColor: backColor}}>
				<AddWrapper>
					...
					<WrappBtnsChoose>
						<button onClick={() => addToCartHandler(indexOfCard, data)}>
							Ok
						</button>
