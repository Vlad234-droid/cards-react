...
const ModalDelete = ({
	...
	const deleteCardHandler = (indexOfCard, cart, setCart) => {
		const dataFilter = cart.filter((_, i) => i !== indexOfCard);
		const infoOfCard = dataFilter;

		// localStorage.setItem(indexOfCard, JSON.stringify(infoOfCard));
		// const t = JSON.parse(localStorage.getItem(0));
		setCart([...infoOfCard]);
		setDisplay(!display);
	};
		...
		<CardShadow className="shadow" onClick={exitCardHandler}>
			<Card style={{backgroundColor: backColor}}>
				<AddWrapper>
					...
					<WrappBtnsChoose>
						<button
							onClick={() => deleteCardHandler(indexOfCard, cart, setCart)}
						>
							Ok
						</button>
